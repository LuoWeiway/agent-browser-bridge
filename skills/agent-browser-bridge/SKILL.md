...
1. **Chrome 已加载扩展**：`chrome://extensions` → 开发者模式 → 加载已解压的扩展程序 → 选 `E:/work/2026v/codex-browser-bridge/extension`
2. **守护进程**：首次调用时自动后台拉起，监听 `127.0.0.1:18888`，无需手动启动
3. 扩展图标为绿色即已连接；红灯时先在终端跑一次 `list` 唤醒守护进程，再点扩展里的「重新连接」
...
**方式 B — MCP 工具**（可选，注册后可用，工具名 `browser_*`）
