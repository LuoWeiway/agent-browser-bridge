...
description: 通过本地桥接控制用户「正在使用的真实 Chrome 浏览器」——复用全部登录态与内网权限，后台静默操作、不抢焦点。支持读取已登录的内网页面（OA、工单、Jira、看板）、列出/切换标签页、模拟点击、悬停、按键(Enter/Esc/Tab/快捷键)、表单填写、下拉选择、异步等待、控制台报错抓取与截图。触发词：浏览器、标签页、读取网页、抓取页面、看板、内网系统、OA、管理后台、登录态、网页截图、点击按钮、填表、按键、悬停、控制台日志、自动化操作页面。
version: 2.3.0
...
3. **全链路自检诊断**：运行 `node server.js doctor`，全绿灯即代表所有链路（含本地安全令牌）畅通。
...
node "$BRIDGE" press Enter             # 按回车键
node "$BRIDGE" hover ".dropdown-menu"  # 悬停展开
node "$BRIDGE" wait "#table-loaded"    # 等待异步渲染
node "$BRIDGE" logs                    # 查看页面控制台报错
```

**方式 B — MCP 工具**（已注册，共 15 个标准工具）
`browser_read` / `browser_list_tabs` / `browser_click` / `browser_hover` / `browser_fill` / `browser_press_key` / `browser_select` / `browser_wait_for` / `browser_get_console_logs` / `browser_scroll` / `browser_screenshot` / `browser_navigate` / `browser_eval` / `browser_close_tab` / `browser_clean_group`
...
3. node server.js hover/click/fill   → 执行交互
4. node server.js press Enter        → 按键触发提交或搜索
5. node server.js wait "<选择器>"     → 等待页面异步数据刷新完成
6. node server.js shot "<关键词>"     → 回读截图验证结果
7. node server.js clean              → 收尾，关掉本次产生的后台标签组
```
...
| 命令 | 说明 |
|---|---|
| `doctor [--fix]` | 🩺 全链路健康诊断与自愈修复（含 CSRF 安全令牌检测） |
...
| `hover <选择器或text=文本> [关键词]` | 鼠标悬停，触发下拉菜单、Tooltip 或悬浮操作项 |
| `fill <选择器> <值> [关键词]` | 填表单，自动派发 input/change 事件（适配 React/Vue 受控组件） |
| `press <按键名> [selector] [关键词]` | 模拟按键，如 `Enter`、`Escape`、`Tab`、方向键等 |
| `select <selector> <值> [关键词]` | 在原生 `<select>` 下拉列表中按 value 或文本选择选项 |
| `wait <selector> [超时ms] [关键词]` | 显式等待异步元素加载或变为可见 |
| `logs [关键词] [级别]` | 查看页面控制台日志与未处理异常 (`error`/`warn`/`all`) |
